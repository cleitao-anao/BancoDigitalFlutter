...
EXCEPTION WHEN OTHERS THEN
  ...
  SET status = 'FAILED',
      updated_at = now(),
      description = COALESCE(description, '') || ' Erro: ' || SQLERRM
  WHERE id = v_transaction_id;
  
  RETURN jsonb_build_object(
    'success', false,
    'message', SQLERRM,
    'transaction_id', COALESCE(v_transaction_id::text, 'none')
  );
END;
$$;

-- Concede permissão para a função ser executada por usuários autenticados
GRANT EXECUTE ON FUNCTION public.make_pix_transfer(uuid, text, decimal, text) TO authenticated;

-- Adiciona um log para confirmar que a função foi criada
DO $$
BEGIN
  RAISE NOTICE 'Função make_pix_transfer criada com sucesso';
END $$;
